<template>
    <div v-if="isPostsLoading != true" class="results__content__documents__cards">
        <div v-for="doc in this.displayedDocs" :key="doc" class="results__content__documents__cards__card">
            <div class="results__content__documents__cards__card-header">
                <span v-text="formatDate(doc.ok.issueDate) + ' ' + doc.ok.source.name"></span>
                <h4>{{ doc.ok.title.text }}</h4>
                <span class="results__content__documents__cards__card-attr-container"><span
                        class="results__content__documents__cards__card-attr">Новости</span></span>
            </div>
            ...
            <div class="results__content__documents__cards__card__footer">
                <button class="results__content__documents__cards__card__footer-button"
                    @click="redirectToSource(doc.ok.url)" style="cursor: pointer;">Читать в источнике</button>
...
.results__content__documents__cards__card-attr-container {
    margin-top: auto;
}
